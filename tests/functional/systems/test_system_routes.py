...
def test_create_post(client, init_system_database):
    response = client.post('/systems/create', data=dict(name='DifferentSystem', system_health='Unhealthy', priority=4, language='Java', description='An interesting description'), follow_redirects=True)
...
def test_edit_post(client, init_system_database):
    response = client.post('/systems/edit?system_id=1', data=dict(name='TestyMcTestFaceChanged', system_health='Unhealthy', priority=4, language='Java', description='An interesting description', system_id=1), follow_redirects=True)
