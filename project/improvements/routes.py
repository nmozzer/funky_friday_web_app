...
improvement = Blueprint(
    "improvement", __name__, template_folder="templates", static_folder="static"
)
...
@login_required
def improvements():
    all_improvements = db.session.query(Improvement).order_by(
        desc(Improvement.created_at)
    )
    try:
...
def create_post():
    ...
    try:
        return redirect(
            url_for("system.view", system_id=system_id, improvement_add="successful")
        )
    except Exception as e:
        abort(500)
...
def edit():
    ...
    try:
        return render_template(
            "edit_improvement.html", improvement=improvement, system_id=system_id
        )
    except Exception as e:
        abort(500)
...
def edit_post():
    ...
    Improvement.query.filter_by(id=improvement_id).update(
        dict(name=name, description=description, beans=beans, user_id=current_user.id)
    )
    ...
    try:
        return redirect(
            url_for("system.view", improvement_edit="successful", system_id=system_id)
        )
    except Exception as e:
        abort(500)
...
def delete():
    ...
    if current_user.type == "admin":
        ...
        db.session.commit()
        return redirect(
            url_for("system.view", improvement_delete="successful", system_id=system_id)
        )

    try:
        return redirect(
            url_for(
                "system.view", improvement_delete="unsuccessful", system_id=system_id
            )
        )
    except Exception as e:
        abort(500)
